  ...
  computeTableCols: function(dest) {
    jQuery(dest)
      ...
      .each(function() {
        ...
        var cols = 0;
        jQuery(jQuery.find("dest td,th", jQuery("tr", table)[0])).each(function(
          td
        ) {
          cols += Number(
            Number(jQuery(td).attr("colspan")) ? jQuery(td).attr("colspan") : 1
          );
        });
